...
Lemma subst_conv Γ e l τ κ' x l' τ1 τ2 :
  <[x:=(l', τ1)]>Γ ⊢ e :{l} τ ->
  Γ ⊢ τ2 :: κ' ->
  τ1 ≡ τ2 ->
  x ∉ dom Γ ->
...
Lemma kinding_subst_conv Γ τ κ κ' x l' τ1 τ2 :
  <[x:=(l', τ1)]>Γ ⊢ τ :: κ ->
  Γ ⊢ τ2 :: κ' ->
  τ1 ≡ τ2 ->
  x ∉ dom Γ ->
...
Proof.
  apply typing_kinding_mutind; intros; subst;
    ...
    (* Another round of inversion lemmas. *)
    repeat type_inv;
    repeat kind_inv;
    (* Massage equivalence assumptions. *)
    repeat
      match goal with
      | H : ?τ ≡ ?τ |- _ => clear H
      | H1 : ?τ1 ≡ ?τ, H2 : ?τ ≡ ?τ2 |- _ =>
          assert (τ1 ≡ τ2) by (etrans; [ apply H1 | apply H2 ]);
          try clear H1 H2 τ
      end;
    (* Massage labels. *)
    repeat
      match goal with
      | H : ⊤ ⊑ ?l |- _ =>
          assert (l = ⊤) by (apply top_inv; apply H); clear H
      end;

    (* Derive equivalence for the sub-expressions. *)
    repeat simpl_whnf_equiv;
  ...
  rewrite !subst_fresh by shelve;
  eapply pared_equiv_open1; simpl_cofin?;
  repeat apply_pared_equiv_congr;
  eauto using lc, open_respect_lc, typing_lc, kinding_lc
