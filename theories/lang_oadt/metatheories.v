...
Corollary obliviousness_open Σ x l τ' e τ s s' :
  gctx_typing Σ ->
  Σ; ({[x:=(l, τ')]}) ⊢ e :{⊥} τ ->
  x ∉ fv τ' ->
  Σ; ∅ ⊢ s :{l} τ' ->
  Σ; ∅ ⊢ s' :{l} τ' ->
  s ≈ s' ->
  trace_indistinguishable Σ <{ {x↦s}e }> <{ {x↦s'}e }>.
Proof.
  intros.
...
Corollary obliviousness_open_obliv_val Σ x l τ' e τ v v' :
  gctx_typing Σ ->
  Σ; ({[x:=(l, τ')]}) ⊢ e :{⊥} τ ->
  x ∉ fv τ' ->
  Σ; ∅ ⊢ v :{l} τ' ->
  ...
  Σ; ∅ ⊢ τ' :: *@O ->
  val v -> val v' ->
  trace_indistinguishable Σ <{ {x↦v}e }> <{ {x↦v'}e }>.
Proof.
